...
import poc.ecommerce.dao.UserRepository;
...
public class SecurityServiceImpl implements SecurityService, UserDetailsService {
	...
	private static class JwtUtils {
		...
		private static String generateToken(final Long userId, final String username, final String userRole) {
			...
			String jwtToken = Jwts.builder().setSubject(userId.toString()).claim(TOKEN_CLAIM_USERNAME, username)
					.claim(TOKEN_CLAIM_ROLES, userRole).setIssuedAt(now).setNotBefore(now).setExpiration(exp)
		...
		private static Claims parseToken(String token) {
			return Jwts.parser().setSigningKey(TOKEN_SECRET_KEY).parseClaimsJws(token.replace(TOKEN_PREFIX, ""))
					.getBody();
		}
	}
